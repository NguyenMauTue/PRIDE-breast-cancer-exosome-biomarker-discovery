...
sink("../data/MissingData_Summary.txt")
summary(fit_det)
sink()
...
ggsave("../results/detection_bias_plot.png", plot = pMissing, width = 8, height = 6, dpi = 300)

cat(paste("logistic regression analysis demonstrated a strong positive association between protein abundance and detection probability with β =",
          coef(fit_det)["MeanIntensity"],
          "and p =",
          summary(fit_det)$coefficients[2, 4]))
...
cat(paste0("Approximately ", prop_3v0, "% of proteins exhibited complete 3v0 missingness patterns."))
